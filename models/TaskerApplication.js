...
class TaskerApplication {
  ...
  static async updateStatus(id, status, reviewerId, note=null) {
    await executeQuery("UPDATE TaskerApplications SET status=@param1, reviewed_at=GETDATE(), reviewer_id=@param2, note=@param3 WHERE application_id=@param4", [status, reviewerId, note, id]);
