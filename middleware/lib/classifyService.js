...
const keywordsConfig = {
  ...
  'sua_chua_dieu_hoa': {
    positive: [
      // Vietnamese core terms
      'dieu hoa','may lanh','dien lanh','sua dieu hoa','sua chua dieu hoa','sua chua may lanh','sua chua dien lanh','bao tri dieu hoa','bao tri may lanh',
      // Broader electrical repair contexts that are acceptable for AC service background
      'dien dan dung','dien gia dung','sua chua dien dan dung','sua chua dien gia dung','sua chua dien','tho dien',
      // English variants
      'air conditioner','airconditioner','air conditioning','aircon','ac repair','hvac','hvac repair'
    ],
    // Lower minScore slightly and cap denominator to avoid penalizing long synonym lists
    minScore: 0.15,
    denomCap: 6
  },
  've_sinh_nha_cua': {
...
function classifyService({ services = [], text }) {
  ...
  for (const [slug, cfg] of Object.entries(keywordsConfig)) {
    ...
    // Optional denominator cap to prevent long lists driving score too low
    const denom = cfg.denomCap ? Math.min(positives.length, cfg.denomCap) : positives.length;
